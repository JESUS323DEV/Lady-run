-- Oferta PERMANENTE (a diferencia del resto de precios de marcos/skins, que son momentaneos): Fondo-1
-- se queda para siempre a mitad de precio (10 -> 5 Huesin), item_id propio porque el servidor cobra
-- por item_id fijo, no por lo que calcule el cliente (ver ladyRunAvatarFramesCatalog.js).
...
begin
  case p_item_id
    ...
    when 'marco_base' then d_chapas := 0; d_tavern_coins := 30; d_huesin := 0;
    when 'marco_fondo' then d_chapas := 0; d_tavern_coins := 0; d_huesin := 10;
    when 'marco_fondo_oferta' then d_chapas := 0; d_tavern_coins := 0; d_huesin := 5;
